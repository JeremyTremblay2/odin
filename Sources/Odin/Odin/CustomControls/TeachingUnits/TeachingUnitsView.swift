    ...
    public var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 0) {
                ...
                ForEach($odinVM.teachingUnitsVM) { $unit in
                    HStack(alignment: .center, spacing: 0) {
                        ViewWithLineView(view: TeachingUnitView(teachingUnit: unit))
                        NavigationLink(destination: TeachingUnitDetailView(odinVM: odinVM, teachingUnitVM: unit)) {
